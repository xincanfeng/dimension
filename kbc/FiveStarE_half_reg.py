...
5*E_half_reg model:
a, b, c, d in C
no reg for imaginary part
'''


class FiveStarE_half_reg(KBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int], rank: int,
            init_size: float = 1e-3
    ):
        super(FiveStarE_half_reg, self).__init__()
        self.sizes = sizes
        self.rank = rank
...
def transformation(embeddings, x, flag, rank):
    ...
    if flag == "score":
        ...
        return (             
                ...
                torch.sqrt(re_relation_a ** 2 + re_relation_c ** 2 + re_relation_b ** 2 + re_relation_d ** 2) * 2,
