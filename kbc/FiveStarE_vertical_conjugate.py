...
'''
5*E_down_conjugate model:
a, b, c, d in C,
c = conjugate(a)
d = conjugate(b)
i.e.
re_relation_c = re_relation_a
im_relation_c = -im_relation_a
re_relation_d = re_relation_b
im_relation_d = -im_relation_b
'''


class FiveStarE_vertical_conjugate(KBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int], rank: int,
            init_size: float = 1e-3
    ):
        super(FiveStarE_vertical_conjugate, self).__init__()
        self.sizes = sizes
        self.rank = rank
...
def transformation(embeddings, x, flag, rank):
    ...
    5*E_down_conjugate model: needs only 4 relation parameters:
                              re_relation_c = re_relation_a, im_relation_c = -im_relation_a,
                              re_relation_d = re_relation_b, im_relation_d = -im_relation_b
    '''
    ...
    # 'c'h
    re_score_c = re_head * re_relation_a + im_head * im_relation_a
    im_score_c = -re_head * im_relation_a + im_head * re_relation_a

    # 'c'h + 'd'
    re_score_dn = re_score_c + re_relation_b
    im_score_dn = im_score_c - im_relation_b
    ...
    if flag == "score":
        # Re(<h_r, t>): denominator is the same, so don't need to calculate it
