...
class FiveStarE_conjugate(KBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int], rank: int,
            init_size: float = 1e-3
    ):
        super(FiveStarE_conjugate, self).__init__()
        self.sizes = sizes
        self.rank = rank

        '''
        5*E model: needs 8*rank parameters
        5*E_all_conjugate model: only need 4*rank parameters
        '''
...
def transformation(embeddings, x, flag, rank):
    ...
    5*E_all_conjugate model: needs only 4 relation parameters:
