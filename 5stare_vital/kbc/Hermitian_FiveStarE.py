...
class Hermitian_FiveStarE(KBCModel):

    def __init__(
            self, sizes: Tuple[int, int, int], rank: int,
            init_size: float = 1e-3
    ):
        super(Hermitian_FiveStarE, self).__init__()
        self.sizes = sizes
        self.rank = rank
...
'''
   # param flag : "scores", "queries", "forward"
...
def calculation(embeddings, x, flag, rank):
    ...
    # ditto, of relation
    # 0 - 1 re-a
    re_relation_a = rel[:, : rank]
    # 1 - 2 re-b 2-3 im-b
    # re_relation_c = re_relation_b
    # im_relation_c = -im_relation_b
    re_relation_b, im_relation_b = rel[:,
                                       rank:2 * rank], rel[:, 2 * rank:3 * rank]
    # 3 - 4 re-d
    re_relation_d = rel[:, 3 * rank: 4 * rank]
