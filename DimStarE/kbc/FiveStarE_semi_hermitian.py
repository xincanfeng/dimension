...
class FiveStarE_semi_hermitian(KBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int], rank: int,
            init_size: float = 1e-3
    ):
        super(FiveStarE_semi_hermitian, self).__init__()
        self.sizes = sizes
        self.rank = rank

        '''
        5*E model: needs 8*rank parameters
        5*E_semi_hermitian model: only need 6*rank parameters
        '''
...
def transformation(embeddings, x, flag, rank):
    ...
    re_relation_a, im_relation_a, re_relation_b, im_relation_b, re_relation_d, im_relation_d = \
        rel[:, :rank], rel[:, rank:2*rank], rel[:, 2*rank:3*rank], rel[:, 3*rank:4*rank], \
        rel[:, 4*rank:5*rank], rel[:, 5*rank:]
    ...
    if flag == "score":
        ...
        return (
                ...
                torch.sqrt(re_relation_a ** 2 + im_relation_a ** 2 + re_relation_b ** 2 +
                           (-im_relation_b) ** 2 + re_relation_b ** 2 + im_relation_b ** 2 +
